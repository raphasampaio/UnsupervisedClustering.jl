...
function fit!(balanced::BalancedKmeans, data::AbstractMatrix{<:Real}, result::KmeansResult)
    ...
    result.iterations = balanced.max_iterations
    result.converged = false

    clusters_size = zeros(Int, k)
    distances = zeros(k, n)

    for iteration in 1:balanced.max_iterations
        previous_objective = result.objective

        pairwise!(balanced.metric, distances, result.clusters, data', dims = 2)
        ...
        # stopping condition
        if change < balanced.tolerance || n == k
...
function fit(balanced::BalancedKmeans, data::AbstractMatrix{<:Real}, initial_clusters::AbstractVector{<:Integer})::KmeansResult
    n, d = size(data)
    ...
    initialize!(result, data, initial_clusters, verbose = balanced.verbose)

    fit!(balanced, data, result)

    return result
end

function fit(balanced::BalancedKmeans, data::AbstractMatrix{<:Real}, k::Integer)::KmeansResult
    n, d = size(data)
    ...
    unique_data, indices = try_sampling_unique_data(balanced.rng, data, k)
    initialize!(result, unique_data, indices, verbose = balanced.verbose)
