...
mutable struct HardResult <: Result
    ...
    function HardResult(assignments::Vector{Int}, centers::Vector{Vector{Float64}}, L::Vector{LowerTriangular{Float64, Matrix{Float64}}}, totalcost::Float64)
...
mutable struct SoftResult <: Result
    ...
    function SoftResult(assignments::Vector{Int}, weights::Vector{Float64}, centers::Vector{Vector{Float64}}, covariances::Vector{Matrix{Float64}}, L::Vector{LowerTriangular{Float64, Matrix{Float64}}}, totalcost::Float64)#, model::Model)
...
function Base.copy(result::SoftResult)
    return SoftResult(copy(result.assignments), copy(result.weights), deepcopy(result.centers), deepcopy(result.covariances), deepcopy(result.L), result.totalcost)#, result.model)
end
...
function update_weights!(child::HardSphericalResult, parent1::HardSphericalResult, parent2::HardSphericalResult, assignment::Vector{Int})
end

function update_weights!(child::SoftSphericalResult, parent1::SoftSphericalResult, parent2::SoftSphericalResult, assignment::Vector{Int})
end

function update_weights!(child::HardResult, parent1::HardResult, parent2::HardResult, assignment::Vector{Int})
end
